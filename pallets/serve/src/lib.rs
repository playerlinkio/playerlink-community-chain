...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add_serve(
			...
			let escrow_account: <T as frame_system::Config>::AccountId = Self::account_id(who.clone().encode(),collection_id,collection_serve_id.saturating_sub(1));
			...
			Collections::<T>::mutate(collection_id,|collection|{
				if let Some(ref mut c) = collection {
					c.serve_number = c.serve_number.checked_add(1).ok_or(Error::<T>::NumberOverflow)?;
				}
		...
		pub fn registered_use_server_certificate(
			...
			UseServeEscrowAccount::<T>::insert((who_account_id32.clone(), collection_id), serve_id,escrow_account);
			...
			ServiceCertificate::<T>::insert((who_account_id32.clone(), collection_id), serve_id, certificate.clone());
			...
			Collections::<T>::mutate(collection_id,|collection|{
				...
				collection.registered_serve_number_all = collection.registered_serve_number_all.checked_add(1).ok_or(Error::<T>::NumberOverflow)?;
				CollectionServe::<T>::mutate(collection_id, serve_id, |serve|{
					let serve = serve.as_mut().ok_or(Error::<T>::CollectionNotFound)?;
					serve.registered_serve_number = serve.registered_serve_number.checked_add(1).ok_or(Error::<T>::NumberOverflow)?;
					Ok(())
				})
			})
		}
		...
		pub fn check(
			...
			ensure!(message.len() >= T::MessageMinBytes::get() as usize,Error::<T>::LessThanMessageMinBytes);
			log::debug!(target: "serve", "Certificate = {:?}", message.len());

			let certificate = Self::parse_json(message.clone()).map_err(|_| Error::<T>::ParseJsonFailed)?;
			log::debug!(target: "serve", "Certificate = {:?}", certificate);

			let c = ServiceCertificate::<T>::get((address.clone(),certificate.collection_id),certificate.serve_id).ok_or(Error::<T>::ServiceCertificateNotFound)?;
			...
			ServiceCertificate::<T>::mutate((address.clone(),certificate.collection_id), certificate.serve_id, |c|{
				...
				old_certificate.times = old_certificate.times.checked_add(1).ok_or(Error::<T>::NumberOverflow)?;
...
impl<T: Config> Pallet<T> {
	pub fn parse_json(message: Vec<u8>)-> Result<Certificate,Error<T>> {
		...
		let account_id_str = from_utf8(account_id_vec.as_slice()).map_err(|_| Error::<T>::ParseJsonFailed)?;
		...
		let collection_id_str = from_utf8(collection_id.as_slice()).map_err(|_| Error::<T>::ParseJsonFailed)?;
		let collection_id_u32 = (collection_id_str.parse::<u32>()).map_err(|_| Error::<T>::ParseJsonFailed)?;
		let serve_id = message[flag[1] as usize + 13..flag[2] as usize - 1].to_vec();
		let serve_id_str = from_utf8(serve_id.as_slice()).map_err(|_| Error::<T>::ParseJsonFailed)?;
